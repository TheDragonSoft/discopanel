...
// Wraps an ingress net.Listener to parse PROXY protocol v1/v2 headers so
// connections routed through an external edge (VPS tunnel, Pangolin, HAProxy)
// present the real client address. When trustedCIDRs is non-empty, headers are
// only honored for upstreams inside those CIDRs; others are ignored.
