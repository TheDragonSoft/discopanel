<script lang="ts">
	...
	import type { Player, OnlinePlayer, PlayerSession } from '$lib/proto/discopanel/v1/player_pb';
	import {
							...
							<div class="flex flex-wrap gap-1.5">
								...
								{#if onlinePlayers.length > 12}
									<Badge variant="secondary" class="text-xs"
										>+{onlinePlayers.length - 12} more</Badge
									>
								{/if}
							</div>
						{/if}
			...
			<div class="flex flex-col gap-2 sm:flex-row">
				...
				>
					<SelectTrigger class="sm:w-56">
						<span
							>{serverFilter
								? servers.find((s) => s.id === serverFilter)?.name || 'All Servers'
								: 'All Servers'}</span
						>
					</SelectTrigger>
					<SelectContent>
							...
							<Table>
								...
								<TableBody>
									{#each players as player (player.id)}
										<TableRow class="cursor-pointer" onclick={() => openPlayerDetail(player)}>
								...
								>
									<div class="min-w-0 flex-1">
										<div class="flex items-center gap-2">
											...
											{#if !session.leftAt}
												<Badge variant="outline" class="text-[10px] {ONLINE_BADGE_CLASS}">
													Active
												</Badge>
											{/if}
										</div>
									...
									<div class="flex items-center gap-4 text-xs text-muted-foreground">
										...
										<span class="font-mono">
											↓ {formatBytes(Number(session.bytesIn))} ↑ {formatBytes(
